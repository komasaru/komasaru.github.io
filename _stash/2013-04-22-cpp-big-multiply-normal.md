...
今回は、「標準（筆算）方式」による多桁同士の乗算アルゴリズムを C++ で実装してみました。
...
「筆算方式」の考え方は、小学校で習ったとおりであるが、今回は配列は下位から取るようにしている。

![BIG_MULTIPLY]({{site.baseurl}}/images/2013/04/22/BIG_MULTIPLY.png "BIG_MULTIPLY")

### 2. C++ ソース作成

例として、以下のようにソースを作成した。概要は以下のとおり。

* 上記の説明では1個の配列で4桁を扱うようにしているが、以下のソースでは1個の配列で1桁を扱うようにしている。
* 計算可能な桁数は2のべき乗桁としている。  
  （後に筆算方式以外のアルゴリズムを利用することを考えて）
* 繰り上がり処理は、最後にまとめて行う（但し、非常に大きい乗算桁数（数千万桁以上）では桁あふれを起こすので注意）
...
* [Gist - C++ source code to multiply big-digit values.](https://gist.github.com/komasaru/670f762fc7cfaf4a9b79061115a6c49f "Gist - C++ source code to multiply big-digit values.")
...
``` text
$ g++ -Wall -O2 -o multiply_normal multiply_normal.cpp
```
...
![MULTIPLY_NORMAL_CPP]({{site.baseurl}}/images/2013/04/22/MULTIPLY_NORMAL_CPP.png "MULTIPLY_NORMAL_CPP")
...
単純に多桁同士の乗算を計算するだけなら「筆算方式」で充分ですが、もっと桁数を増やしたり計算速度を上げる必要がある場合は、別のアルゴリズムを使用するようです。
