...
- [mk-mode SITE : DB - 高温注意情報（気象庁防災情報XML）](http://www.mk-mode.com/rails/jmaxml_db/kohon "mk-mode SITE : DB - 高温注意情報（気象庁防災情報XML）")

今までは受信したデータをある程度整形して公開していただけでしたので、若干現状を把握しにくい部分もありました。

前回までの一覧同様、最新の「高温注意情報」を一覧で確認できるようなページを作成したので紹介します。

<!--more-->

### 1. 「一覧 - 高温注意情報」ページ

以下のようなページを作成・公開した。（地方高温注意情報と府県高温注意情報を同じページに並べて掲載）

![JMAXML_KOHON]({{site.baseurl}}/images/2014/05/19/JMAXML_KOHON.png "JMAXML_KOHON")

実際のサイトは以下。よろしければ、ご参照ください。

- [mk-mode SITE : 一覧 - 高温注意情報（気象庁防災情報XML）](http://www.mk-mode.com/rails/jmaxml_list/lst_kohon "mk-mode SITE : 一覧 - 高温注意情報（気象庁防災情報XML）")
