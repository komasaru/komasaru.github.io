...
そこで、最新バージョンをソースをビルドしてインストールしてみました。（Linux Mint 17 64bit へ）

<!--more-->

### 0. 前提条件

- Linux Mint 17 での作業を想定しているが Debian 系はどれも同様と思われる。
...
$ mkdir build
$ cd build
$ ../gcc-4.9.1/configure --prefix=/usr/local/gcc-4.9.1 --enable-languages=c,c++,objc,fortran --disable-multilib --disable-bootstrap
...
# sudo ln -s /usr/local/gcc-4.9.1/bin/gfortran /usr/local/bin/gfortran49
```
...
（Linux Mint 環境では、 Object でなく NSObject を継承するために libgnustep-base-dev がインストール済みであること）
...
``` text
$ g++49 -o TestSumM TestSumM.m -lobjc -lgnustep-base -I/usr/include/GNUstep
$ ./TestSumM
1 + ... + 100 = 5050
```
...
今回のようにソースをビルドしてインストールするのではなく、リポジトリを追加してインストールするのなら、以下のようにすればよいらしい。（当方未確認）
...
また、新しいバージョンの GCC でコンパイルしたものがライブラリの関係で正常に動作しないこともあるかも知れない。（"libstdc++.so.6" 関連等）  
