...
- [mk-mode SITE : DB - 台風解析・予報（気象庁防災情報XML）](http://www.mk-mode.com/rails/jmaxml_db/typhoon_analyze "mk-mode SITE : DB - 台風解析・予報（気象庁防災情報XML）")

今までは受信したデータをある程度整形して公開していただけでしたので、若干現状を把握しにくい部分もありました。

前回までの一覧同様、最新の「台風解析情報」を一覧で確認できるようなページを作成したので紹介します。（予報の部分はデータの構造上同じ一覧するのは困難（不可能ではないが見た目が煩雑になりそう）なため除外）
...
![JMAXML_TYPHOON_ANALYZE]({{site.baseurl}}/images/2014/05/25/JMAXML_TYPHOON_ANALYZE.png "JMAXML_TYPHOON_ANALYZE")

マウスオーバで簡単な Tips をポップアップするようにしている。（UA, ブラウザによっては表示されないかも）  
また、24時間後・48時間後の予報情報については発表日時リンクをクリックして詳細情報ページヘ遷移してご確認下さい。

実際のサイトは以下。よろしければ、ご参照ください。

- [mk-mode SITE : 一覧 - 台風解析情報（気象庁防災情報XML）](http://www.mk-mode.com/rails/jmaxml_list/lst_typhoon_analyze "mk-mode SITE : 一覧 - 台風解析情報（気象庁防災情報XML）")
...
今回の一覧は「台風解析・予報情報(3日予報)」の実況部分の情報によるものでした。  
場合によっては「台風解析・予報情報(5日予報)」情報も発表されますが、実況部分は内容が重複することになるので「台風解析・予報情報(5日予報)」の情報は一覧にしていません。

その他のページについても順次対応していこうと考えている次第です。
