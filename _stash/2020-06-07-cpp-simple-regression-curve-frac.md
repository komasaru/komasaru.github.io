...
* [Fortran - 2つの配列から単回帰曲線（分数（逆数）回帰モデル）計算！]({{site.baseurl}}/2019/08/23/fortran95-simple-regression-curve-frac "Fortran - 2つの配列から単回帰曲線（分数（逆数）回帰モデル）計算！")
...
{% highlight c linenos %}
#ifndef REGRESSION_CURVE_FRAC_FILE_HPP_
#define REGRESSION_CURVE_FRAC_FILE_HPP_
...
{% highlight c linenos %}
#ifndef REGRESSION_CURVE_FRAC_CALC_HPP_
#define REGRESSION_CURVE_FRAC_CALC_HPP_
...
public:
  ...
  bool reg_curve_frac(double&, double&);             // 単回帰曲線（分数（逆数）回帰）の計算
};
 ...
 * @retval     false 失敗
 */
bool Calc::reg_curve_frac(double& a, double& b) {
  unsigned int i;      // loop インデックス
  double s_ix  = 0.0;  // sum(1 / x)
  double s_ix2 = 0.0;  // sum((1 / x) * (1 / x))
  double s_y   = 0.0;  // sum(y)
  double s_yix = 0.0;  // sum(y * 1 / x)
  double x     = 0.0;  // x 計算用
  double y     = 0.0;  // y 計算用
  double ix    = 0.0;  // 1 / x 計算用

  try {
    // データ数
    cnt = data.size();

    // sum(1 / x), sum((1 / x) * (1 / x)), sum(y), sum(y * 1 / x)
    for (i = 0; i < cnt; i++) {
      x  = data[i][0];
      y  = data[i][1];
      ix = 1.0 / x;
    ...
    // 行列1行目
    mtx.push_back({(double)cnt, s_ix, s_y});
    // 行列2行目
    mtx.push_back({s_ix, s_ix2, s_yix});
...
File: `regression_curve_frac.cpp`

{% highlight c linenos %}
/***********************************************************
  単回帰曲線（分数（逆数）回帰モデル）計算
  : y = a + b / x
  : 連立方程式をガウスの消去法で解く方法

    DATE          AUTHOR          VERSION
    2020.06.01    mk-mode.com     1.00 新規作成
  ...
  try {
    ...
    if (argc != 2) {
      std::cerr << "[ERROR] Number of arguments is wrong!\n"
                << "[USAGE] ./regression_curve_frac <file_name>"
                << std::endl;
      return EXIT_FAILURE;
    }
...
* [Gist - C++ source code to calculate a simple regression curve(frac).](https://gist.github.com/komasaru/8ed4f4078425e16ba7fb05d44c829e3b "C++ source code to calculate a simple regression curve(frac).")
...
regression_curve_frac: regression_curve_frac.o file.o calc.o
  g++ $(gcc_options) -o $@ $^

regression_curve_frac.o : regression_curve_frac.cpp
  g++ $(gcc_options) -c $<
...
run : regression_curve_frac
  ./regression_curve_frac

clean :
  rm -f ./regression_curve_frac
  rm -f ./*.o
...
``` text
$ ./regression_curve_frac data.txt
説明変数 X  目的変数 Y
   83.0000    183.0000
   71.0000    168.0000
   64.0000    171.0000
...
![REGRESSION_CURVE_FRAC]({{site.baseurl}}/images/2020/06/07/REGRESSION_CURVE_FRAC.png "REGRESSION_CURVE_FRAC")
