...
- [mk-mode SITE : DB - 気象警報・注意報（気象庁防災情報XML）](http://www.mk-mode.com/rails/jmaxml_db/keiho "mk-mode SITE : DB - 気象警報・注意報（気象庁防災情報XML）")

今までは受信したデータをある程度整形して公開していただけでしたので、若干現状を把握しにくい部分もありました。

前回までの一覧同様、最新の「気象警報・注意報」を一覧で確認できるようなページを作成したので紹介します。
...
![JMAXML_KEIHO]({{site.baseurl}}/images/2014/05/10/JMAXML_KEIHO.png "JMAXML_KEIHO")
...
（追記：2014-05-24）
「気象警報・注意報（気象特別警報・警報・注意報）」情報の他に「気象特別警報報知」という情報も存在するが、今回の「一覧 - 気象警報・注意報」に含まれる情報と重複するため「気象特別警報報知」としての一覧は作成していない。
（追記：ここまで）

可能ならば、その他のページについても今回の処理を流用して順次対応していこうと考えている次第です。
