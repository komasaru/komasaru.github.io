...
* [Fortran - 2つの配列から単回帰曲線（ルート回帰モデル）計算！]({{site.baseurl}}/2019/08/17/fortran95-simple-regression-curve-sqrt "Fortran - 2つの配列から単回帰曲線（ルート回帰モデル）計算！")
...
{% highlight c linenos %}
#ifndef REGRESSION_CURVE_SQRT_FILE_HPP_
#define REGRESSION_CURVE_SQRT_FILE_HPP_
...
{% highlight c linenos %}
#ifndef REGRESSION_CURVE_SQRT_CALC_HPP_
#define REGRESSION_CURVE_SQRT_CALC_HPP_
...
public:
  ...
  bool reg_curve_sqrt(double&, double&);             // 単回帰曲線（ルート回帰）の計算
};
 ...
 * @retval     false 失敗
 */
bool Calc::reg_curve_sqrt(double& a, double& b) {
  unsigned int i;     // loop インデックス
  double s_x   = 0.0;  // sum(x)
  double s_xs  = 0.0;  // sum(sqrt(x))
  double s_y   = 0.0;  // sum(y)
  double s_xsy = 0.0;  // sum(sqrt(x) * y)
  double x     = 0.0;  // x 計算用
  double y     = 0.0;  // y 計算用
  double xs    = 0.0;  // sqrt(x) 計算用

  try {
    // データ数
    cnt = data.size();

    // sum(x), sum(sqrt(x)), sum(y), sum(sqrt(x) * y
    for (i = 0; i < cnt; i++) {
      x  = data[i][0];
      y  = data[i][1];
      xs = std::sqrt(x);
    ...
    // 行列1行目
    mtx.push_back({(double)cnt, s_xs, s_y});
    // 行列2行目
    mtx.push_back({s_xs, s_x, s_xsy});
...
File: `regression_curve_sqrt.cpp`
  ...
  try {
    ...
    if (argc != 2) {
      std::cerr << "[ERROR] Number of arguments is wrong!\n"
                << "[USAGE] ./regression_curve_sqrt <file_name>"
                << std::endl;
      return EXIT_FAILURE;
    }
...
* [Gist - C++ source code to calculate a simple regression curve(sqrt).](https://gist.github.com/komasaru/a567d620120596dc97c198767d70afe2 "C++ source code to calculate a simple regression curve(sqrt).")
...
regression_curve_sqrt: regression_curve_sqrt.o file.o calc.o
  g++ $(gcc_options) -o $@ $^

regression_curve_sqrt.o : regression_curve_sqrt.cpp
  g++ $(gcc_options) -c $<
...
run : regression_curve_sqrt
  ./regression_curve_sqrt

clean :
  rm -f ./regression_curve_sqrt
  rm -f ./*.o
...
``` text
$ ./regression_curve_sqrt data.txt
説明変数 X  目的変数 Y
   83.0000    183.0000
   71.0000    168.0000
   64.0000    171.0000
...
![REGRESSION_CURVE_SQRT]({{site.baseurl}}/images/2020/05/28/REGRESSION_CURVE_SQRT.png "REGRESSION_CURVE_SQRT")
