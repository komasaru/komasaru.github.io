...
* [Fortran - 2つの配列から単回帰曲線（指数（ab指数）回帰モデル）計算！]({{site.baseurl}}/2019/08/29/fortran95-simple-regression-curve-exp "Fortran - 2つの配列から単回帰曲線（指数（ab指数）回帰モデル）計算！")
...
\frac{\partial S}{\partial b} &=& \frac{2}{b}\sum_{i=1}^{N}(\log{a}+x_{i}\log{b} - \log{y_i})x_i= 0
\end{eqnarray*}
$$
...
{% highlight c linenos %}
#ifndef REGRESSION_CURVE_EXP_FILE_HPP_
#define REGRESSION_CURVE_EXP_FILE_HPP_
...
{% highlight c linenos %}
#ifndef REGRESSION_CURVE_EXP_CALC_HPP_
#define REGRESSION_CURVE_EXP_CALC_HPP_
...
public:
  ...
  bool reg_curve_exp(double&, double&);              // 単回帰曲線（指数（ab指数）回帰）の計算
};
 ...
 * @retval     false 失敗
 */
bool Calc::reg_curve_exp(double& a, double& b) {
...
File: `regression_curve_exp.cpp`

{% highlight c linenos %}
/***********************************************************
  単回帰曲線（指数（ab指数）回帰モデル）計算
  : y = a * b**x
  : 連立方程式をガウスの消去法で解く方法

    DATE          AUTHOR          VERSION
    2020.06.03    mk-mode.com     1.00 新規作成
  ...
  try {
    ...
    if (argc != 2) {
      std::cerr << "[ERROR] Number of arguments is wrong!\n"
                << "[USAGE] ./regression_curve_exp <file_name>"
                << std::endl;
      return EXIT_FAILURE;
    }
...
* [Gist - C++ source code to calculate a simple regression curve(exp).](https://gist.github.com/komasaru/02d74256866d710fb8566c47c454514e "C++ source code to calculate a simple regression curve(exp).")
...
![REGRESSION_CURVE_EXP]({{site.baseurl}}/images/2020/06/17/REGRESSION_CURVE_EXP.png "REGRESSION_CURVE_EXP")
