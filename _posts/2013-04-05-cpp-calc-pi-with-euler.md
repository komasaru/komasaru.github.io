...
今回も、同様に $$\arctan$$系の公式である「オイラーの公式」を使用して、円周率 $$\pi$$ を計算してみました。
...
### 1. オイラー(Euler)の公式について

Arctan 系の公式なので、マチンの公式等と考え方は同じ。  
数式が多いので $$\TeX$$ で記載。

![PI_EULER_1]({{site.baseurl}}/images/2013/04/05/PI_EULER_1.png "PI_EULER_1")

![PI_EULER_2]({{site.baseurl}}/images/2013/04/05/PI_EULER_2.png "PI_EULER_2")

ちなみに、$$\tan ^{-1} = \arctan$$ と置き換えてもよい。

なお、ここではオイラーの公式の証明はしない。（証明方法は何種類かあります）
また、上記ではオイラーの公式をテイラー（グレゴリー）展開しているが、オイラーは別の方法で展開してオイラーの公式を使用しているらしい。

そして、上記の方法で算出した計算項数では、収束の速い方は無駄に多く計算していることになる。  
それを防ぎたかったら、各 Arctan 毎に必要な項数分だけ計算して、最後に合算する方法を取ると良いだろう。

### 2. C++ ソース作成

例として、以下のようにソースを作成した。  
基本的に、マチンの公式や Klingenstierna の公式による計算プログラムと同様だが、今回のオイラーの公式では項の分子が `1` でないのでその点に注意。

* 計算桁数を入力するようにした。
* 計算する項数もプログラム内で計算させるようにした。
* 計算結果をテキストファイルに出力するようにした。

File: `calc_pi_euler.cpp`
...
* [Gist - C++ source code to compute Pi with Euler's formula.](https://gist.github.com/komasaru/588427f7c05f4212bbf547383c75be75 "Gist - C++ source code to compute Pi with Euler's formula.")

### 3. C++ ソースコンパイル

``` text
$ g++ calc_pi_euler.cpp -o calc_pi_euler
```
...
また、マチンの公式や Klingenstierna の公式で計算・出力した結果と一致しました。  
...
また、Arctan 系の公式はどれも同様なアルゴリズムで項数・係数が異なるだけなので、汎用化してみたい次第です。
