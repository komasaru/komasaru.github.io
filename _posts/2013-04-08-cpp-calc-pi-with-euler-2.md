...
今回も、同様に Arctan 系の公式である「オイラーの公式(2)」を使用して、円周率 $$\pi$$ を計算してみました。  
ちなみに、正確には「オイラーの公式(2)」という名前の公式ではありません。数ある「オイラーの公式」のうち、今回当方が紹介するのが２つ目という意味の (2) です。

当然、プログラミン言語そのものが保有している三角関数は使用しません。級数展開して計算します。（多桁の円周率計算では、全く無意味ですから。但し、常用対数は影響がないので関数を使用）

また、Arctan 系公式は項数や係数が異るだけなので、１つのプログラムで組むこととしました。
...
![PI_EULER2_1]({{site.baseurl}}/images/2013/04/08/PI_EULER2_1.png "PI_EULER2_1")

![PI_EULER2_2]({{site.baseurl}}/images/2013/04/08/PI_EULER2_2.png "PI_EULER2_1")
...
* [Gist - C++ source code to compute Pi with arctan's formula.(old ver.)](https://gist.github.com/komasaru/507609b9fce17fa22f807ca00224a132 "Gist - C++ source code to compute Pi with arctan's formula.(old ver.)")

### 3. C++ ソースコンパイル

``` text
$ g++ calc.h calc.cpp calc_pi_arctan_old.cpp -o calc_pi_arctan_old
```
